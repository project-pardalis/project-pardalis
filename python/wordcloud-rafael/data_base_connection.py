...
def create_component_connection_with_metrica(component_name: str, fk_maquina: int, fk_empresa: int):
    ...
    for metrica in metricas:
        ...
        component = get_component(component_name, fk_empresa, fk_maquina)[
            "idComponente"]
...
def clear_components(hashMaquina: str):
    ...
    for component in components:
        if AMBIENTE == 0:
            command = "DELETE FROM Componente WHERE idComponente = {0} AND fkMaquina = {1} AND fkEmpresa = {2};".format(
                component["idComponente"], computer["idMaquina"], computer["fkEmpresa"])

        else:
            command = "DELETE FROM [pardalis].[dbo].[Componente] WHERE idComponente = {0} AND fkMaquina = {1} AND fkEmpresa = {2};".format(
                component["idComponente"], computer["idMaquina"], computer["fkEmpresa"])
        run_sql_command(command)
...
def SO_isNull(hash_maquina: str):
    if AMBIENTE == 0:
        result = run_sql_command(
            "SELECT sistemaOperacional FROM Maquina WHERE hashMaquina = '{0}';".format(rename_hash(hash_maquina)))
    else:
        result = run_sql_command(
            "SELECT [sistemaOperacional] FROM [pardalis].[dbo].[Maquina] WHERE hashMaquina = '{0}';".format(rename_hash(hash_maquina)))
