...
function getDataLeituraMetrica(id, dataChamado, dataAntesChamado, metrica) {
    instrucao = `select valorLeitura, nomeMetrica,dataColeta, unidadeDeMedida from Leitura JOIN Maquina   on idMaquina=${id}  and fkMaquina=idMaquina  Join Metrica on fkMetrica=idMetrica and nomeMetrica='${metrica}' and dataColeta BETWEEN '${dataAntesChamado}' and '${dataChamado}';`
...
async function addChamado(assuntoChamado, descricaoChamado, assuntoChamado, categoriaChamado, emailUsuario, fkMaquina) {
    ...
    if (verifyIdUsuario[0] == undefined) {
        ...
        database.executar(`INSERT INTO Chamado (assuntoChamado, descricaoChamado,  categoriaChamado, dataChamado, isAberto, prioridadeChamado, fkUsuario, fkMaquina ) VALUES ( '${assuntoChamado}',  '${descricaoChamado}', '${categoriaChamado}',GETDATE() ,1,'Normal' ,${verifyIdUsuario[0].idUsuario}, ${fkMaquina} ) ;`)
...
// adicionar on nomeMaquina = maquina e ajustar o sistema para pegar o nome da maquina no chamado
function listarChamados(conditionalId) {
    if (conditionalId == false) {
        instrucao = "SELECT  idChamado, assuntoChamado, descricaoChamado, categoriaChamado, dataChamado, nomeUsuario, emailUsuario,  isAberto, prioridadeChamado, idUsuario,fkMaquina,nomeMaquina FROM Chamado JOIN Usuario on idUsuario=fkUsuario JOIN Maquina on idMaquina=fkMaquina"

        return database.executar(instrucao)
    }
    else {
        instrucao = `SELECT idChamado, assuntoChamado, descricaoChamado, categoriaChamado, dataChamado, nomeUsuario, emailUsuario,   nomeMaquina, hashMaquina, isAberto, idUsuario, fkMaquina FROM Chamado  JOIN Usuario on idChamado=${conditionalId} JOIN Maquina on fkMaquina=idMaquina  `
...
function getDadosMaquina(id) {
    ...
    if (id.indexOf('.com') != -1) {
        ...
        instrucao = `select idMaquina, nomeMaquina, sistemaOperacional, dataCriacao, hashMaquina from Maquina Join Usuario ON emailUsuario='${id}'`
    }
    else {
